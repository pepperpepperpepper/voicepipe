...
1) **Split and de-duplicate the giant CLI**
...
2) **Introduce a “recording backend” abstraction**
...
3) **Consolidate recorder implementations**
...
4) **Unify transcription path (and optionally leverage transcriber daemon)**
...
5) **Centralize “type text” support + add Wayland options**
...
6) **Tighten runtime dir/socket permissions for non-systemd environments**
...
7) **Remove install/service duplication**
   ...
   - Refactor:
     ...
     - Decide one canonical system for units:
       - either “render units in Python only” (preferred)
       - or “templates only”
     - Optional: introduce a `voicepipe.target` so starting/stopping “Voicepipe” is one systemd command.
