...
def test_live_zwingli_audio_round3_execute_enabled_but_shell_disallowed_returns_error(
    ...
    assert inner.get("action") == "execute"
    handler_meta = inner.get("handler_meta") or {}
    assert handler_meta.get("enter") is True

    _assert_contains_ordered_tokens(out, ["echo", "hello", "world"], label="output")


def test_live_zwingli_audio_round3_execute_sleep_2_enabled_returns_command(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.delenv("VOICEPIPE_SHELL_ALLOW", raising=False)
    ...
    _assert_output_does_not_start_with_trigger(out, commands=commands)
    _assert_contains_ordered_tokens(out, ["sleep", "2"], label="output")
