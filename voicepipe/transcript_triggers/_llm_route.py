...
_VERB_DESCRIPTIONS: dict[str, str] = {
    ...
    "call": 'call someone by NAME. For a normal PHONE call (business/place/person; client checks contacts, else web lookup) pass a bare "<name + any location>" e.g. "Sukhothai Hotel Shanghai", "Sam Spears". For a WhatsApp/Signal call to a saved contact pass "name=<person>; via=whatsapp|signal" and add "; video=true" for a video call — e.g. "call mom on whatsapp" → "name=mom; via=whatsapp". Use when the user names who to call instead of giving digits.',
    ...
    "navigate": 'turn-by-turn maps directions to ONE specific destination ("navigate to Paris", "walk me to the library", "drive to the airport"). args: "[<mode>] to <destination>" — ALWAYS put the travel mode FIRST when the user implies one ("walk me to X" → "walking to X", "drive to X" → "driving to X", "bike to X" → "bicycling to X"). mode ∈ {driving, walking, bicycling, transit}. Use `places` instead for "nearby/nearest/find X" with no specific destination.',
    "places": 'open a MAP showing a places search near the current location ("show me nearby gas stations", "find coffee near me", "where\'s the nearest pharmacy"). args: just the place type/name — "gas stations", "pharmacy", "coffee". Use this (not navigate) whenever the user wants to FIND/SEE options rather than route to one named place.',
    "calendar": 'create a calendar event. args: "title=<event>; day=<today|tomorrow|monday..sunday|YYYY-MM-DD>; time=<7:30am|15:00>" OR "title=<event>; in=<duration>" for relative (e.g. "in 2 hours"). Omit day/time fields you don\'t have. e.g. "dentist Friday 3pm" → "title=dentist; day=friday; time=3pm"; "standup tomorrow 9am" → "title=standup; day=tomorrow; time=9am"; "call mom in 2 hours" → "title=call mom; in=2 hours". Title-only ("title=lunch") is fine.',
    "note": 'jot a quick note to self (opens Keep / a notes app pre-filled). args: the note text, e.g. "note to self buy milk" → "buy milk", "remember to call the dentist" → "call the dentist".',
    "weather": 'show the weather. args: a location, or empty for the current location. e.g. "what\'s the weather" → "", "weather in Paris" → "Paris".',
    "dnd": 'toggle Do Not Disturb / silence the phone. args: "on" (also "silence my phone", "do not disturb") or "off" ("turn off do not disturb", "allow notifications").',
    ...
    "media": 'control media playback. args: one of "play", "pause", "next", "previous", "stop" (also: "skip"→next, "resume"→play). e.g. "pause the music" → "pause".',
    "volume": 'media volume. args: "up", "down", "mute", "unmute", or a level 0–100 ("set volume to 30" → "30", "max" → "100"). e.g. "turn it up" → "up".',
    "flashlight": 'toggle the flashlight/torch. args: "on", "off", or empty to toggle. e.g. "turn on the flashlight" → "on".',
    "camera": 'open the camera app. args: "photo" (default), "video", or "selfie". e.g. "take a selfie" → "selfie", "open the camera" → "photo".',
    "screenshot": "take a screenshot. no args.",
    "lock_screen": 'lock the screen / lock the phone. no args.',
