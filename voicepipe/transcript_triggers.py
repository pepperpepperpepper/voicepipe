...
def match_transcript_trigger(
    ...
    *,
    ...
    for raw_trigger, raw_action in triggers.items():
        ...
        if not trigger:
            continue
        action = (raw_action or "").strip().lower() or "zwingli"
...
def _run_zwingli(prompt: str) -> tuple[str, dict[str, Any]]:
...
def apply_transcript_triggers(
    ...
    *,
    ...
    """
    resolved_triggers = get_transcript_triggers() if triggers is None else dict(triggers)
